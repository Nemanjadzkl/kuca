...
function DashboardPage() {
  ...
  const fetchDashboardData = async () => {
    const { data: jobs, error } = await supabase.from('poslovi').select('*, klijenti(naziv)');
    if (jobs) {
    ...
    <div className="p-4 sm:p-8">
      <header className="flex flex-wrap justify-between items-center mb-8 pb-4 border-b">
        ...
        <nav className="flex items-center gap-2 sm:gap-4 mt-4 sm:mt-0">
          <Link href="/clients"><Button variant="link">Klijenti</Button></Link>
          <Link href="/jobs"><Button variant="link">Poslovi</Button></Link>
          <Link href="/reports"><Button variant="link">Izveštaji</Button></Link>
                ...
                <ul>
                  {upcomingJobsList.map(job => (
                    <li key={job.id} className="mb-2 p-2 border-b">
                      ...
                      <p className="text-sm text-gray-500">Klijent: {job.klijenti?.naziv || 'N/A'}</p>
